...
type WorkInterval struct {
	Start string `json:"start"` // 开始 HHmm
	End   string `json:"end"`   // 结束 HHmm
}
...
type Config struct {
	...
	// 米家相关
	MiUser         string `json:"miUser"`         // 米家账号
	MiPass         string `json:"miPass"`         // 米家密码
	MiServiceToken string `json:"miServiceToken"` // 米家serviceToken（登录后自动保存）
	MiSSecurity    string `json:"miSsecurity"`    // 米家ssecurity
	MiUserID       string `json:"miUserId"`       // 米家userId
	MiDeviceID     string `json:"miDeviceId"`     // 米家deviceId
	MiHomeID       string `json:"miHomeId"`       // 选中的家庭ID
	MiHomeOwner    int64  `json:"miHomeOwner"`    // 家庭所有者UID
	MiDeviceDid    string `json:"miDeviceDid"`    // 选中的设备DID
	MiDeviceName   string `json:"miDeviceName"`   // 选中的设备名称（显示用）
	MiSiid         int    `json:"miSiid"`         // 开关属性siid（空调通常为2）
	MiPiid         int    `json:"miPiid"`         // 开关属性piid（空调通常为1）

	// 睡过头闹钟
	OversleepEnable        bool           `json:"oversleepEnable"`        // 是否启用睡过头检测
	WorkIntervals          []WorkInterval `json:"workIntervals"`          // 工作时间区间列表（支持多个）
	WorkOnly               bool           `json:"workOnly"`               // 仅工作日检测
	BarkUrl                string         `json:"barkUrl"`                // Bark推送链接（含key）
	BarkInterval           int    `json:"barkInterval"`           // Bark通知间隔（秒）
	OversleepCheckInterval int    `json:"oversleepCheckInterval"` // 设备状态检查间隔（秒）
	VolRampStart           int    `json:"volRampStart"`           // 渐强起始音量
	VolRampStep            int    `json:"volRampStep"`            // 每次渐强增加的音量
	VolRampInterval        int    `json:"volRampInterval"`        // 音量渐强间隔（秒）
	VolRampMax             int    `json:"volRampMax"`             // 渐强最大音量
}
...
var (
	...
	Cfg = Config{
		...
		// 米家设备开关属性默认值（空调标准miot-spec: siid=2 piid=1）
		MiSiid: 2,
		MiPiid: 1,

		// 睡过头检测默认配置
		OversleepEnable: false,
